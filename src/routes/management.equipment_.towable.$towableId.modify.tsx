...
export const Route = createFileRoute(
  ...
  loader: async ({ params: { towableId } }) => {
    const towable = await queryClient.ensureQueryData(
      getFindTowableQueryOptions({ towableId }),
    )
...
function TowableModify() {
  ...
  const updateTowable = useMutation({
    mutationFn: (towable: Towable) =>
      api.towables.updateTowable({ towableId, towable }),
    onSuccess: () => {
