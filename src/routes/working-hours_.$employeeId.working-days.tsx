...
import { Box, Button, Checkbox, FormControlLabel, IconButton, Paper, Stack, TextField, Typography, styled } from "@mui/material";
import { createFileRoute, useNavigate } from "@tanstack/react-router";
import AggregationsTable from "components/working-hours/aggregations-table";
import WorkDay from "components/working-hours/work-day";
import { useTranslation } from "react-i18next";
import { RouterContext } from "./__root";
import WorkDayTableHeader from "components/working-hours/work-day-table-header";
import ChangeLog from "components/working-hours/change-log";

export const Route = createFileRoute("/working-hours/$employeeId/working-days")({
  component: WorkingDays,
  beforeLoad: (): RouterContext => ({
    breadcrumbs: ["workingHours.title", "workingHours.workingDays.title"],
  }),
});
...
const TableContainer = styled(Stack, {
  label: "working-days-table-container",
})(({ theme }) => ({
  ...
  const renderToolbar = () => {
    return (
      <Stack direction="row" justifyContent="space-between">
        <ToolbarContainer>
          <IconButton onClick={() => navigate({ to: "/working-hours" })} title={t("tooltips.backToWorkingHours")}>
            <ArrowBack />
          </IconButton>
          <TextField
            sx={{ maxWidth: 300 }}
        ...
        <Stack>
          <TableHeader>
            ...
            <Stack spacing={4} direction="row" alignItems="center">
              <FormControlLabel
                control={
                  <Checkbox title="Merkitse kaikki tarkistetuiksi" />
          ...
          <TableContainer>
            <WorkDayTableHeader />
            {
              ...
              Array.from({ length: 14 }).map((_, index) => (
                <WorkDay key={`${index}_${Math.random()}`} />
              ))
            }
          </TableContainer>
        </Stack>
