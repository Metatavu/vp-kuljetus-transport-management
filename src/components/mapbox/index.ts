...
namespace Mapbox {
  ...
  export const getSuggestions = async (query: string, sessionToken: SessionToken) => {
    const suggestions = await doRequest<MapboxSuggestionResponse>(`/search/searchbox/v1/?q=${query}&session_token=${sessionToken}&language=${DEFAULT_COUNTRY}&limit=${DEFAULT_LIMIT}`);
  ...
  export const retrieveSuggestion = async (id: string, sessionToken: SessionToken) => {
    const feature = await doRequest<MapboxFeatureResponse>(`/search/searchbox/v1//${id}?session_token=${sessionToken}`);
